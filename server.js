...
app.post('/api/webhook', async (req, res) => {
  ...
  try {
    switch (action) {
      case 'get_items': {
        const result = await db.query('SELECT * FROM cakes_catalog');
        return res.json(result.rows);
      }

      case 'admin_add': {
        const { name, price, image_url, stock, color, cake_type } = body;
        const result = await db.query(
          `INSERT INTO cakes_catalog (name, price, image_url, stock, color, cake_type) 
           VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
          [name, price, image_url, stock, color, cake_type]
        );
      ...
      case 'pay': {
        ...
        const productRes = await db.query('SELECT * FROM cakes_catalog WHERE id = $1', [item.productId]);
        ...
        await db.query(
          `INSERT INTO cakes_orders 
          (order_id, user_id, amount, username, status, cake_name, address, street, house, apartment, entrance, intercom, comment, items)
      ...
      case 'update_stock': {
        ...
        const result = await db.query('UPDATE cakes_catalog SET stock = $1 WHERE id = $2 RETURNING *', [totalStock, id]);
        return res.json(result.rows[0] || { status: 'error' });
      }

      case 'check_status': {
        const { orderId } = body;
        const result = await db.query('SELECT status FROM cakes_orders WHERE order_id = $1', [orderId]);
      ...
      case 'get_orders': {
        const { userId } = body;
        const result = await db.query('SELECT * FROM cakes_orders WHERE user_id = $1', [userId]);
        return res.json(result.rows);
      }
      ...
      case 'cancel_order': {
        ...
        const updateRes = await db.query(
            "UPDATE cakes_orders SET status = 'cancelled' WHERE order_id = $1 RETURNING *", 
            [orderId]
        );
...
Юзернейм: @${username}
Состав заказа:
  Наименование: ${items?.name || order.cake_name}
...
Комментарий к заказу: ${order.comment || 'Нет'}`;
      ...
      case 'update_order_status': {
        ...
        const result = await db.query('UPDATE cakes_orders SET status = $1 WHERE order_id = $2 RETURNING *', [status, orderId]);
        return res.json(result.rows[0] || { status: 'error' });
      }

      case 'get_all_orders': {
        const result = await db.query('SELECT * FROM cakes_orders');
        return res.json(result.rows);
      }

      case 'admin_delete': {
        const { id } = body;
        await db.query('DELETE FROM cakes_catalog WHERE id = $1', [id]);
  ...
  try {
    ...
    if (event.event === 'payment.succeeded' || event.event === 'payment.waiting_for_capture') {
        ...
        const orderRes = await db.query(
            "UPDATE cakes_orders SET status = 'paid' WHERE order_id = $1 RETURNING *", 
            [paymentObj.id]
        );
        ...
        if (orderRes.rows.length > 0) {
            ...
            if (meta.productId && meta.count) {
                await db.query('UPDATE cakes_catalog SET stock = stock - $1 WHERE id = $2', [meta.count, meta.productId]);
            }

            // Отправляем уведомление в Telegram о новом оплаченном заказе
            const text = `Новый заказ (ОПЛАЧЕН): 
${order.cake_name} / ${meta.count || 1}шт
